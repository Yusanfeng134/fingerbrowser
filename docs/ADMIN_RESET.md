...
当需要恢复旧版本本地账号或离线缓存中的 legacy 管理员记录时，可以使用本地 CLI 脚本。云账号上线后，该脚本不作为云账号密码重置入口；云账号恢复应走云端账号流程。

该脚本只更新本机 SQLite 数据库中的 `app_users` 记录，不会删除环境、代理、审计、Cookie、缓存、本地配置或浏览器 profile 目录。
